...
public class StrungBowItem extends BowItem {
  ...
  public static float getPowerForTime(float drawSpeed, int timeHeld) {
  ...
  public static float getPowerForTime(ItemStack stack, RegistryAccess registryAccess, int timeHeld) {
    ...
    return getPowerForTime(stats.get(BowStat.DRAW_SPEED), timeHeld);
  }
  ...
  public boolean releaseUsing(ItemStack itemStack, Level level, LivingEntity entity, int remainingTime) {
    if (entity instanceof Player player) {
      ...
      if (projectile.isEmpty()) {
        ...
        BowStats stats = BowAssembler.assemble(itemStack, level.registryAccess());
        int timeHeld = this.getUseDuration(itemStack, entity) - remainingTime;
        float pow = getPowerForTime(stats.get(BowStat.DRAW_SPEED), timeHeld);
        if ((double) pow < 0.1) {
          return false;
        } else {
          ...
          if (level instanceof ServerLevel serverLevel && !firedProjectiles.isEmpty()) {
            float arrowSpeed = pow * 3.0F * stats.get(BowStat.VELOCITY);
            float inaccuracy = 1.0F / stats.get(BowStat.ACCURACY);
            this.shoot(serverLevel, player, player.getUsedItemHand(), itemStack, firedProjectiles, arrowSpeed, inaccuracy, pow == 1.0F, null);
          }

          float shotVolume = 1.0F / stats.get(BowStat.STEALTH);
          level.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ARROW_SHOOT, SoundSource.PLAYERS,
              shotVolume, 1.0F / (level.getRandom().nextFloat() * 0.4F + 1.2F) + pow * 0.5F);
  ...
  protected Projectile createProjectile(Level level, LivingEntity shooter, ItemStack weapon, ItemStack projectile, boolean isCrit) {
    AbstractArrow arrow = (AbstractArrow) super.createProjectile(level, shooter, weapon, projectile, isCrit);
    BowStats stats = BowAssembler.assemble(weapon, level.registryAccess());
    arrow.setBaseDamage(2.0 * stats.get(BowStat.DAMAGE));
    if (!arrow.isCritArrow() && level.getRandom().nextFloat() < stats.get(BowStat.CRIT_CHANCE)) {
